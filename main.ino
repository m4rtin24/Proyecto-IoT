...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .mqtt-dot { width: 12px; height: 12px; border-radius: 50%; background: #00ff88; display: inline-block; margin-right: 8px; }
    .timestamp { text-align: center; color: #555; font-size: 0.8em; margin-top: 10px; }
  </style>
</head>
<body>
  <h1>🔥 Detector de Gas MQ-2</h1>
  <p class="subtitle">Universidad de la Sabana 2026 — Sistema IoT</p>

  <div class="mqtt-status">
    <span><span class="mqtt-dot"></span>MQTT: broker.hivemq.com</span>
  ...
  <div class="chart-container">
    <div class="chart-title">📈 Historial PPM (últimos 30 valores)</div>
    <canvas id="chart" height="80"></canvas>
  </div>

  <p class="timestamp" id="ts">Última actualización: --</p>
  ...
  <script>
    ...
    async function fetchData() {
      try {
        ...
        document.getElementById('ts').textContent  = 'Última actualización: ' + d.timestamp;
...
void loop() {
  ...
  if (wifiOK) {
    ...
    if (now - lastHealth > 30000) {
      lastHealth = now;
      String health = "{\"status\":\"ok\",\"uptime\":" + String(millis()/1000) + ",\"ip\":\"" + WiFi.localIP().toString() + "\"}";
